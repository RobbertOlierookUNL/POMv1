...
const filter = (data, {filter, level, reference, value}) => {
	const filteredData = [];
...
export const useFilterableData = (dataSet) => {
	const [arrayOfFilters] = useGlobal(state => state.arrayOfFilters);
	const filteredData =  React.useMemo(() => {
		let iteratingData = [...dataSet];
		for (var filterObject of arrayOfFilters) {
			iteratingData = filter(iteratingData, filterObject);
		}
...
const getDropDownParameters = (processedObj, myMeta, key, parameters, level) => {
	if (myMeta[key]?.filtertype === "dropdown") {
		...
		if (parameters[key][level].options) {
			!parameters[key][level].options.includes(processedObj[key]) && parameters[key][level].options.push(processedObj[key]);
		} else {
			parameters[key][level].options = [processedObj[key]];
		}
	}
	return parameters;
};
